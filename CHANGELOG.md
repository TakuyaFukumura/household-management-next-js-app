...
- `src/app/components/BudgetTable.tsx`: 予算設定テーブルの収入・支出の種別を色分け表示

## [0.18.0] - 2026-08-30

### 変更

- `src/app/components/AggregationTable.tsx`: カテゴリ別代表値表の表示を整理
  - 「対象月数」と重複する代表値列を削除
  - 代表値がともに0円のカテゴリを非表示化
  - 収入・支出の種別を色分け表示
...
- `src/app/components/BudgetSummaryCards.tsx`: 予算画面のカード文言を見直し
    - `収入予算` → `想定収入` に変更（一般ユーザーにとって直感的な表現へ改善）
    - `支出予算` → `想定支出` に変更（`想定収入` と統一感のある対表現に）
- `src/app/components/BudgetPieChart.tsx`: 円グラフタイトルを `支出予算の割合` → `予算割合` に変更（簡潔な表現へ改善）
- `src/app/components/BudgetBarChart.tsx`: 棒グラフタイトルを `収支予算` → `予算収支` に変更（意味が明確な表現へ改善）
- `package.json`: バージョンを `0.15.0` から `0.15.1` に更新

### 削除

- `docs/予算画面_文言見直し仕様.md`: 実装完了のため削除

## [0.15.0] - 2026-04-04

### 変更

- `src/app/components/TransactionTable.tsx`: 収支一覧テーブルの日付列表示を改善
    - `YYYY-MM-DD` 形式の日付を日のみ（`DD日`）の形式で表示するよう変更
    - 画面上部の `MonthNavigator` で年月が明示されているため、一覧テーブルの年・月を省略することで可読性を向上
    - ソート処理（`b.date.localeCompare(a.date)`）は元の `YYYY-MM-DD` 文字列を使用しており、変更なし
- `package.json`: バージョンを `0.14.0` から `0.15.0` に更新

### 削除

- `docs/収支一覧画面_改善仕様.md`: 実装完了のため削除
