...
describe('validateRow', () => {
    describe('正常系', () => {
        it('支出の行を正しくパースできる', () => {
            const result = validateRow(['2024-01-05', '食費', '支出', '5000', 'スーパー'], 2);
        ...
        it('メモが空欄の行を正しくパースできる', () => {
            const result = validateRow(['2024-01-05', '食費', '支出', '5000', ''], 2);
            expect(result.error).toBeNull();
    ...
    describe('異常系', () => {
        it('必須フィールドが不足している場合はエラーを返す', () => {
            const result = validateRow(['2024-01-05', '食費', '支出'], 2);
        ...
        it('種別が収入・支出以外の場合はエラーを返す', () => {
            const result = validateRow(['2024-01-05', '食費', '不明', '5000', ''], 2);
            expect(result.transaction).toBeNull();
            expect(result.error?.message).toContain('種別が不正です');
        });

        it('金額が0の場合はエラーを返す', () => {
            const result = validateRow(['2024-01-05', '食費', '支出', '0', ''], 2);
            expect(result.transaction).toBeNull();
            expect(result.error?.message).toContain('金額が不正です');
        });

        it('金額が負の場合はエラーを返す', () => {
            const result = validateRow(['2024-01-05', '食費', '支出', '-100', ''], 2);
            expect(result.transaction).toBeNull();
            expect(result.error?.message).toContain('金額が不正です');
        });

        it('金額が小数の場合はエラーを返す', () => {
            const result = validateRow(['2024-01-05', '食費', '支出', '5000.5', ''], 2);
            expect(result.transaction).toBeNull();
            expect(result.error?.message).toContain('金額が不正です');
        });

        it('金額が文字列の場合はエラーを返す', () => {
            const result = validateRow(['2024-01-05', '食費', '支出', 'abc', ''], 2);
            expect(result.transaction).toBeNull();
            expect(result.error?.message).toContain('金額が不正です');
        });
    });
